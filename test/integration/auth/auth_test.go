...
`
	csrCreateUrl := "/apis/certificates.k8s.io/v1/certificatesigningrequests"

	token := BobToken
	createCSRBodyBytes := bytes.NewReader([]byte(csrBody))
	createCSRReq, err := http.NewRequest("POST", s.URL+csrCreateUrl, createCSRBodyBytes)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	createCSRReq.Header.Set("Authorization", fmt.Sprintf("Bearer %s", token))
	createCSRReq.Header.Set("Impersonate-User", "alice")
	createCSRReq.Header.Set("Impersonate-UID", "1")
	func() {
		resp, err := transport.RoundTrip(createCSRReq)
		if err != nil {
			t.Logf("posting csr")
			t.Fatalf("unexpected error: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("unexpected error creating certificatesigningrequest: %v", err)
		}
		var resultBodyMap map[string]interface{}
		err = json.Unmarshal(body, &resultBodyMap)
		if err != nil {
			t.Fatalf("unexpected error unmarshaling resultBody: %v", err)
		}
		// get spec.uid
		spec := resultBodyMap["spec"].(map[string]interface{})
		uid := spec["uid"]
		if uid == nil {
